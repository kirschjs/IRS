...
C      CHECK OB MATRIXELEMENT FUER SPINDIFFERENZ MOEGLICH IST
  ...
  131 IF(LT.ne.1.or.KT.ne.2) then
            IFT = 0
            goto 90
      endif
      IFT=2
...
1131  GOTO (100,44), KK
   31 IF(LT.ne.1.or.KT.ne.2) then
            IFT = 0
            goto 90
      endif
      IF(MLKC.LE.LKC) GOTO 33
...
C NKC/2 integer division, i.e. reminders are discarded
   32 IF(LT.ne.1.or.KT.ne.2) then
            IFT = 0
            goto 90
      endif
      IF(NKC-2*(NKC/2).GT.0) GOTO 43
...
C     NKC BEI TENSOR  1=++,2=+0,3=0+,4=+-,5=00,6=-+,7=0-,8=-0,9=--
   35 IF(LT.ne.1.or.KT.ne.2) then
            IFT = 0
            goto 90
      endif
      GO TO (61,61,62,63,62,61,62,63,63),NKC
...
C     ISO-TENSOR TENSOR OPERATOR
      ...
      NALG(LT,3)=MOD(NALG(LT,3)+1,4)+1
      GOTO 99
   69 IF(LT.ne.1.or.KT.ne.2) then
            IFT = 0
            goto 90
      endif
      IFT=2 - 2*((NALG(KT,3)-1)/2) - 2*((NALG(LT,3)-1)/2)
