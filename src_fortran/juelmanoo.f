...
C     LETZTER DREHIMPULS IST OPERATOR
      AMUL=FLOAT(MUL)
      write(6,*)'f=',F
C      if(mkc.ne.1) F=F*F7(LV(NH6)+1)/FLOAT((-1)**LV(NH6))
C      if(mkc.ne.1) F=1
      write(6,*)'f=',F
      DO 780 KZL=1,K1
...
40    CONTINUE
     ...
     1 KH=1,KZAHL)      
C      WRITE (6,*)(JQ(KH),INDPO(KH),EPO(KH),(MVK(IH,KH),IH=1,IQM),
C     1 KH=1,KZAHL)      
      DO 50 IH=1,IQM
      DO 45 KH=1,KZAHL
45    MVK(IH,KH)=0.
