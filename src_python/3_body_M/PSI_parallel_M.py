...
def span_initial_basis(
    ...
    for frg in range(len(lfrags)):
        ...
        #                         dtype=None)
        lit_rw[frg] = [
            logspaceG(start=wiir * (1 - np.random.random() / 10),
                      end=wffr * (1 - np.random.random() / 10),
                      base=(4.05 - 0.5 * np.random.random()),
                      number=nwrel)[::-1] for m in range(len(lit_w[frg]))
        ] if indivRelSets else np.tile(
            logspaceG(start=wiir * (1 - np.random.random() / 10),
                      end=wffr * (1 - np.random.random() / 10),
                      base=(4.05 - 0.5 * np.random.random()),
                      number=nwrel)[::-1], (len(lit_w[frg]), 1))
    ...
    for n in range(len(lit_w)):
        ...
        tmp3 = [lit_rw[nr][bnds[nn]:bnds[nn + 1]] for nn in range(zer_per_ws)]
    ...
    #os.system(set.BINBDGpath + 'DROBER.exe')
    run_external(set.bindingBinDir + 'DROBER.exe')
    he3inquaBS(intwi=widi, relwi=widr, potf=set.nnPotFile)
    ...
    if set.parallel == -1:
        ...
        run_external([
            ...
            '-np',
            str(numberProcesses),
            set.bindingBinDir + 'V18_PAR/mpi_quaf_v7'
        ])
        ...
        run_external(set.bindingBinDir + 'QUAFL_M.exe')
    if set.useV3B:
        he3inquaBS(intwi=widi, relwi=widr, potf=set.nnnPotFile)
        parallel_mod_of_3inqua(lfrags2,
                               sfrags2,
                               infile='INQUA_M',
                               outFileNm='INQUA_M',
        ...
        if set.parallel == -1:
            ...
            run_external(set.bindingBinDir + 'DRQUA_AK_M.exe')
...
#
    ...
    for civ in range(set.civ_size):
        ...
        for frg in range(len(lfrags)):
            ...
            #                         dtype=None)
            lit_rw[frg] = [
                logspaceG(start=wiir * (1.1 - 0.2 * np.random.random()),
                          end=wffr * (1.1 - 0.2 * np.random.random()),
                          base=(3.5 - 0.48 * np.random.random()),
                          number=nwrel) for m in range(len(lit_w[frg]))
            ]
        ...
        for n in range(len(lit_w)):
            ...
            tmp3 = [
                lit_rw[n][bnds[nn]:bnds[nn + 1]] for nn in range(zer_per_ws)
            ]
            sfrags2 += len(tmp2) * [sfrags[n]]
            lfrags2 += len(tmp2) * [lfrags[n]]
            widi += tmp2
...
def end3(para, send_end):
    ...
    he3inquaBS(intwi=para[0], relwi=para[1], potf=para[3].nnPotFile)
    ...
    run_external([
        ...
        '-np',
        str(numberProcesses),
        para[3].bindingBinDir + 'V18_PAR/mpi_quaf_v7'
    ])
    #t1 = time.perf_counter()
    #t1a = time.time()
    ...
    if para[3].useV3B:
        he3inquaBS(intwi=para[0], relwi=para[1], potf=para[3].nnnPotFile)
        parallel_mod_of_3inqua(para[11],
                               para[12],
                               infile='INQUA_M',
                               outFileNm='INQUA_M',
